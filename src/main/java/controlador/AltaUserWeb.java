...
public class AltaUserWeb extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/**
		 * Capturamos todas las variables posibles que nos puedan lanzar desde el post
		 */
		...
		if (filePart != null && filePart.getSize() > 0) {
			fileName = Paths.get(filePart.getSubmittedFileName()).getFileName().toString(); // Obtiene el nombre del
																							// archivo
			...
			try {
				...
				e.printStackTrace(); // Este imprimirá el rastro de la pila de la excepción, incluyendo el mensaje de
										// la excepción.
			}
		...
		if (departamento == null || departamento.equals("Elegir departamento")) {
			...
			// El usuario seleccionó un departamento, y departamento contiene el valor del
		...
		/*
		 * Si en el formulario dependiendo de el estado creamos un objeto tipo
		 * administrador o consultor para poder aplicar futuras funcionalidades
		 */
		if (tipoUsuario == 1) {

			// AdminWeb a1 = new AdminWeb(nombreUsuario,passw);
			AdminWeb t1 = new AdminWeb(nombreUsuario, apellidos, nombre, passw, email, empresaSelect, departamentoId,
					admin);

			// crear objeto con foto
			AdminWeb z1 = new AdminWeb(nombre, apellidos, nombreUsuario, passw, email, fileName, admin, empresaSelect,
					departamentoId);
			// super(nombre, apellidos, nombreUsuario, passw, email, foto, administrador,
